...
namespace AikaEmu.GameServer.Network.Packets.Client
{
    public class RequestToken : GamePacket
    {
        protected override void Read(PacketStream stream)
        {
            ...
            var nation = NationManager.Instance.GetNation(2);
            ...
            {
                var guild = GuildManager.Instance.GetGuild(character.GuildId);
                if (guild != null)
                {
                    Connection.SendPacket(new SendGuildInfo(Connection.Id, guild));
                    // Opcional: Enviar também a lista de membros
                    //Connection.SendPacket(new SendGuildMembers(Connection.Id));
                }
            }
